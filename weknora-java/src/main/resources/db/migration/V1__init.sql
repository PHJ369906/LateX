-- Copied and adapted from migrations/paradedb/00-init-db.sql
-- ParadeDB/PostgreSQL schema with required extensions and tables
...
CREATE INDEX IF NOT EXISTS embeddings_hnsw_3584 ON embeddings USING hnsw ((embedding::halfvec(3584)) halfvec_cosine_ops) WITH (m = 16, ef_construction = 64) WHERE (dimension = 3584);
CREATE INDEX IF NOT EXISTS embeddings_hnsw_798 ON embeddings USING hnsw ((embedding::halfvec(798)) halfvec_cosine_ops) WITH (m = 16, ef_construction = 64) WHERE (dimension = 798);
